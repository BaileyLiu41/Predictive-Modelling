...
def build_dataset(prices: pd.Series,returns: pd.Series, W: int, H: int, theta: float):
    """
    For each time t:
      x_t = [r_{t-W+1}, ..., r_t, mean_5, mean_10, vol_5, vol_10, mom_20]
    ...
    price_values = np.asarray(prices).reshape(-1)
    return_values = np.asarray(returns).reshape(-1)
    price_index = prices.index
    return_index = returns.index

    # Need enough history for:
    # - W lagged returns
    # - 10-day mean/volatility
    # - 20-day momentum
    min_history = max(W, 10, 20)

    for i in range(min_history - 1, len(return_values) - H):

        # Feature window: W returns ending at i
        recent_returns = return_values[i - W + 1:i + 1]

        price_pos_t = i + 1 # present day price is at index i+1
        P_t = price_values[price_pos_t] # present day price

        # Constructing new features: 5-day and 10-day mean returns and volatilities, and 20-day momentum
        mean_5 = np.mean(return_values[i - 4:i + 1])
        mean_10 = np.mean(return_values[i - 9:i + 1])

        vol_5 = np.std(return_values[i - 4:i + 1])
        vol_10 = np.std(return_values[i - 9:i + 1])

        mom_20 = (price_values[price_pos_t] / price_values[price_pos_t - 20]) - 1

        # Concatenate past W returns with new features
        x_t = np.concatenate([
            recent_returns,
            [mean_5, mean_10, vol_5, vol_10, mom_20]
        ])
...
for W in W_grid:
    for H in H_grid:
        for theta in theta_grid:
            ...
            for C in C_grid:
                model = LogisticRegression(C=C, penalty="l2", solver="lbfgs", max_iter=1000, class_weight="balanced")
...
feature_names = [f"return_lag_{j}" for j in range(W, 0, -1)] + ["mean_5", "mean_10", "vol_5", "vol_10", "mom_20"]
weights_df = pd.DataFrame({
    "feature": feature_names,
    "weight": weights
})
print("\nLearned weights by feature:")
print(weights_df)
